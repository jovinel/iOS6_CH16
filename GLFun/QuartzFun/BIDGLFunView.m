...
- (id)initWithCoder:(NSCoder *)coder {
    if (self = [super initWithCoder:coder]) {
        ...
        _useRandomColor = NO;
        _sprite = [[Texture2D alloc] initWithImage:[UIImage imageNamed:@"iphone.png"]];
...
- (void)draw {
    ...
    switch (_shapeType) {
        ...
        case kRectShape: {
            ...
            GLfloat vertices[8];
            GLfloat minX = (_firstTouch.x > _lastTouch.x) ? _lastTouch.x : _firstTouch.x;
            GLfloat minY = (self.frame.size.height - _firstTouch.y > self.frame.size.height - _lastTouch.y) ?
            self.frame.size.height - _lastTouch.y : self.frame.size.height - _firstTouch.y;
            
            GLfloat maxX = (_firstTouch.x > _lastTouch.x) ? _firstTouch.x : _lastTouch.x;
            GLfloat maxY = (self.frame.size.height - _firstTouch.y > self.frame.size.height - _lastTouch.y) ?
        ...
        case kEllipseShape: {
            ...
            for (int i = 0; i <= 720; i += 2) {
                GLfloat xOffset = (_firstTouch.x > _lastTouch.x) ? _lastTouch.x + xradius : _firstTouch.x + xradius;
                GLfloat yOffset = (_firstTouch.y < _lastTouch.y) ? self.frame.size.height - _lastTouch.y + yradius :
        ...
        case kImageShape:
            ...
            [_sprite drawAtPoint:CGPointMake(_lastTouch.x, self.frame.size.height - _lastTouch.y)];
            break;
        default:
            break;
    }
